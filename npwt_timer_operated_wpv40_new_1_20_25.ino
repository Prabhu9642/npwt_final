...
bool cont_run,int_runl,int_runh;
...
void loop() {
  ...
  if (startTimer - previousMillisTimer >= duration) {  // timer code for preesure sensor code. bcs micro pressure sensor will not read the correct values without timer code.
    ...
    Serial.println("totalSecondsB testttttttttttttttttttttttttttttttttttttttttttttttttt:"+ String(totalSecondsB)+"--->"+String(blockage_state));
...
absPressure = pressure_mmhg*0.88;

}


else if(pressure_mmhg>90 &&  pressure_mmhg<=100)
{
...
else if(pressure_mmhg>100 &&  pressure_mmhg<=110)
{
...
else if(pressure_mmhg>110 &&  pressure_mmhg<=120)
{


absPressure = pressure_mmhg*1.05;
...
absPressure = pressure_mmhg*1.12;
...
absPressure = pressure_mmhg*1.23;
...
// else if(cont_run && leakagetest==1) {

//     // battery icon update method based on the voltage values of pb1 and pb0
//    battery_icon(); 


// }

   setText(0x25, 0x00," NPWT021");
...
void battery_icon() {
  ...
  // setText(0x25, 0x00,"0"+String(smoothedVoltage));
  ...
  else if (smoothedVoltage <= 12.2 && smoothedVoltage > 11.5) {
